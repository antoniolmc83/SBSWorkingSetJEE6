...
public class SBSHtmlParser {
	...
	public List<InfoTasaDiaria> parserFilaTasa(String htmlInput){
		...
		if( iCIndex == -1 && fCIndex == -1 ) return resp;
		...
		if( iIndex > 0 && fIndex > 0 ){
			String infoTcea = htmlInput.substring(iIndex, fIndex);			
			String items[] = infoTcea.split("<tr>");
				...
				for(int i=1; i<items.length; i++){
					...
					if( cols!=null ){
						...
						for(int j=0; j<cabecera.size(); j++){
							...
							if( cabecera.get(j).equals("TCEA(*)") ){
								pf.setTcea ( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );		
							}
							if( cabecera.get(j).equals("Cuota") ){
								pf.setCuota ( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );		
							}

							if( cabecera.get(j).equals("Prima(*)") ){
								pf.setPrima( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );		
							}
							
							if( cabecera.get(j).equals("TREA Mínima S/.") ){
								pf.setTreamin( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_SOLES);
							}						

							if( cabecera.get(j).equals("TREA Mínima US$") ){
								pf.setTreamin( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_DOLARES);
							}						

							if( cabecera.get(j).contains( "Tasa de Rendimiento") ){
								pf.setTrea( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );		
							}	
							
							if( cabecera.get(j).startsWith( "TREA (US$ 1000)") ){
								pf.setTrea1000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_DOLARES);
							}
							
							if( cabecera.get(j).startsWith( "TREA (US$ 3000)") ){
								pf.setTrea3000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_DOLARES);
							}
							
							if( cabecera.get(j).startsWith( "TREA (US$ 10000)") ){
								pf.setTrea10000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_DOLARES);
							}	

							if( cabecera.get(j).startsWith( "TREA (S/. 1000)") ){
								pf.setTrea1000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_SOLES);
							}
							
							if( cabecera.get(j).startsWith( "TREA (S/. 3000)") ){
								pf.setTrea3000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_SOLES);
							}
							
							if( cabecera.get(j).startsWith( "TREA (S/. 10000)") ){
								pf.setTrea10000( BigDecimal.valueOf( Double.valueOf(cols.get(j).replace("%", "").trim()))  );
								pf.setMonedaTrea(SBSParametros.MONEDA_SOLES);
							}
						}
